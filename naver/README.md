...
# 작년 10월 인기도 (과거 조회)
python -m naver.main --month 2025-10 --category 패션의류 --html oct2025.html

# 2025-01-01부터 오늘까지 일별 인기도 시계열 → CSV (엑셀에서 바로 열림)
python -m naver.main --from 2025-01-01 --csv history.csv

# 특정 대분류만, 특정 기간만
python -m naver.main --from 2025-01-01 --to 2025-06-30 --category 패션잡화 --csv jabhwa_h1.csv
```

| 옵션 | 기본 | 설명 |
|---|---|---|
| `--period` | 일간 | 인기도 기준 기간 (일간/주간/월간) |
| `--month` | - | 과거 특정 월(YYYY-MM) 조회. 지정 시 `--period`/`--days` 무시하고 그 달만 |
| `--from` / `--to` | - | 일별 인기도 시계열(YYYY-MM-DD~). 랭킹 대신 날짜×키워드 CSV 출력 |
...
### 과거 데이터(`--month`, `--from`)의 한계

데이터랩 **인기도**는 과거 어느 시점이든 조회 가능하지만, 쇼핑 **검색 API는
현재 판매 중인 상품만** 돌려줍니다(시점 재현 불가). 그래서 `--month`로 조회해도
**상품 카드는 항상 오늘 기준**이며, HTML에도 이 사실이 경고 배너로 표시됩니다.

`--from` 시계열의 인기도 값도 **판매실적이 아니라** 카테고리 안에서 기간 전체
최대값을 100 으로 둔 **상대적 검색 인기도**입니다. 키워드 간·날짜 간 비교(추세)에는
유효하지만, 다른 카테고리와의 비교나 절대 규모 해석에는 쓸 수 없습니다.
